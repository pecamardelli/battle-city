// Script assets have changed for v2.3.0 see
// https://help.yoyogames.com/hc/en-us/articles/360005277377 for more information
function macros(){
	#macro	CONFIG_FILE				"config.ini"
	
	#macro	TRUE					"true"
	#macro	FALSE					"false"
	
	#macro	LEVELS					1000
	#macro	PRIZE_PROBABILITY		40
	#macro	PRIZE_CHANGE_TANK_STARS	5
	
	#macro	FADE_IN					0x00
	#macro	FADE_OUT				0x01
	#macro	FADE_TIME				0.3
	
	#macro	STAGE_TANK_NUMBER			15
	#macro	STAGE_TANK_CONCURRENCY		10
	#macro	STAGE_MAX_TANK_CONCURRENCY	10
	#macro	STAGE_TANK_INTERVAL			5
	
	#macro	STAGE_TRUCK_CONCURRENCY		10
	#macro	STAGE_MAX_TRUCK_CONCURRENCY	20
	#macro	STAGE_TRUCK_INTERVAL		10
	
	#macro	MOVE_ACCEL_STEP			0.2
	#macro	MOVE_ANGLE_THRESHOLD	0.999
	
	#macro	INPUT_KEYBOARD			"keyboard"
	#macro	INPUT_JOYSTICK			"joystick"
	#macro	STATUS_HEALTHY			0x00
	#macro	STATUS_OUT_OF_COMBAT	0x01
	
	#macro	MIN_WALL_DISTANCE		50
	#macro	FREEZE_TIME				12
	#macro	INVULNERABILITY_TIME	20
	#macro	INVISIBILITY_TIME		20
	#macro	INVISIBILITY_ALPHA		0.2
	
	#macro	MATERIAL_CONCRETE		0x00
	#macro	MATERIAL_METAL			0x01
	
	// Logger
	#macro LOG_INFO		"[INFO]"
	#macro LOG_SYSTEM	"[SYSTEM]"
	#macro LOG_WARNING	"[WARNING]"
	#macro LOG_ERROR	"[ERROR]"
	#macro LOG_DEBUG	"[DEBUG]"
}